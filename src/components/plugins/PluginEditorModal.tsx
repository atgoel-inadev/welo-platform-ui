...
import { X, Plus, Trash2, ChevronRight, ChevronLeft, CheckCircle, AlertTriangle, XCircle, Loader2, Rocket } from 'lucide-react';
...
export const PluginEditorModal = ({ isOpen, onClose, onSaved, onDeployed, projectId, existing, questions }: Props) => {
  ...
  const [type, setType] = useState<'API' | 'SCRIPT'>(existing?.type ?? 'API');
  const [trigger] = useState<'ON_BLUR'>('ON_BLUR');
  const [onFailBehavior, setOnFailBehavior] = useState(existing?.onFailBehavior ?? 'ADVISORY');
  const [bindAll, setBindAll] = useState((existing?.questionBindings ?? []).length === 0);
  const [selectedQuestions, setSelectedQuestions] = useState<string[]>(existing?.questionBindings ?? []);

  // API config
  const [apiUrl, setApiUrl] = useState(existing?.apiConfig?.url ?? '');
  const [apiMethod, setApiMethod] = useState<'GET' | 'POST' | 'PUT' | 'PATCH'>(existing?.apiConfig?.method ?? 'POST');
  const [apiHeaders, setApiHeaders] = useState<Array<{ key: string; value: string }>>(
    Object.entries(existing?.apiConfig?.headers ?? {}).map(([key, value]) => ({ key, value })),
  );
  const [apiPayload, setApiPayload] = useState(existing?.apiConfig?.payload ?? '{\n  "question": "{{question.text}}",\n  "answer": "{{answer.value}}"\n}');
  const [resultPath, setResultPath] = useState(existing?.apiConfig?.responseMapping?.resultPath ?? 'result');
  const [messagePath, setMessagePath] = useState(existing?.apiConfig?.responseMapping?.messagePath ?? 'message');
  const [timeout, setTimeout2] = useState(existing?.apiConfig?.timeout ?? 5000);
  const [retries, setRetries] = useState(existing?.apiConfig?.retries ?? 0);

  // Script config
  const [scriptCode, setScriptCode] = useState(existing?.scriptCode ?? STARTER_SCRIPT);
  ...
  const handleSaveDraft = async () => {
    ...
    try {
      const dto = buildDto();
      const plugin = isEdit
        ? await pluginService.updatePlugin(projectId, existing!.id, dto)
  ...
  const handleDeploy = async () => {
    if (!isEdit && !existing) {
      alert('Save the plugin as draft first, then deploy.');
      return;
    }
    setDeploying(true);
    try {
      const pluginId = existing!.id;
      const deployed = await pluginService.deployPlugin(projectId, pluginId);
      onDeployed(deployed);
    } catch (err: any) {
  ...
  const handleTest = async () => {
    if (!existing) {
      alert('Save as draft first to test.');
      return;
    }
    setTesting(true);
    setTestResult(null);
    ...
    try {
      // Use a valid UUID format for test task ID
      const result = await pluginService.executePlugin('00000000-0000-0000-0000-000000000000', {
  ...
  const insertVar = (setter: (v: string) => void, current: string, chip: string) =>
    setter(current + chip);
        ...
        <div className="relative bg-white rounded-xl shadow-2xl w-full max-w-4xl">
          ...
          <div className="flex items-center justify-between px-6 py-4 border-b">
            <div>
              <h2 className="text-xl font-bold text-gray-900">
                {isEdit ? `Edit Plugin: ${existing.name}` : 'Create New Plugin'}
              </h2>
              ...
              <div className="space-y-5">
                {!existing && (
                  <div className="bg-amber-50 border border-amber-200 rounded-lg p-3 text-sm text-amber-800">
                    Save as draft first, then come back to test.
                  </div>
                )}
                ...
                {existing && (
                  <>
                    ...
                    <Button
                      ...
                      variant="primary"
                      disabled={testing}
                      icon={testing ? undefined : undefined}
                    >
                      {testing ? (
                    ...
                    <div className="border-t pt-4">
                      <div className="flex items-center justify-between">
                        ...
                        <Button
                          ...
                          icon={Rocket}
                        >
                          {deploying ? 'Deploying...' : `Deploy v${(existing.version ?? 0) + 1}`}
                        </Button>
                      </div>
                    </div>
                  </>
          ...
          <div className="flex items-center justify-between px-6 py-4 border-t bg-gray-50 rounded-b-xl">
            ...
            <div className="flex items-center gap-3">
              ...
              <Button onClick={handleSaveDraft} variant="primary" disabled={saving}>
